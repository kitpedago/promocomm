...
| **Motif d'annulation d'une réservation**           | La liste « Motif annulation » existe dans `/parametres` (table `motif_annulation` côté app — aucune table dans le .bak, elle survit aux réimports) et est semée avec les 4 motifs du combo WinDev fournis par le client le 2026-08-13 (changement situation personnelle / changement de projet / non obtention du financement / modification du calendrier du programme). L'annulation d'une réservation reste en texte libre. | Brancher le champ annulation de la modale Commercialisation sur la liste (en attente : fichier en cours de modification par ailleurs).                                                            |
