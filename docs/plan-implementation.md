...
- **Livré 2026-08-13 (arbre complet)** (migration 0024) : toutes les entrées
  ...
  mail / Notaires). Nouvelles listes : Domaine Stade d'avancement (2), Motif
  annulation (vide, sans table legacy — gérée côté app, survit aux réimports),
  Zonage ABC (3), Stades d'avancement (65 jalons de référence), Archives
  stades (entêtes, 2 — le détail `ArchiveStadeAvancementListe` legacy est vide,
  non repris), Règles d'alerte de stade + États d'alerte + Types de date
  (tables legacy vides, écran prêt), Actions (5), Types de bâtiments (4) et
  intervalles bâtiments/stades (28), Valeur des paramètres (23 clés `Param`,
  valeurs déjà NULL dans le legacy), Catégories (6) / Organismes (9)
  subvention, Modèles de mail (vide, corps RTF non repris — texte simple),
  Fonctions interlocuteurs notaires (+ colonne Fonction sur les
  interlocuteurs), Droits en matrice services × modules lecture seule
  (source : `lib/services.ts`). **Non repris (outillage WinDev)** : Éditeur de
  code, Éditeur de requête, Suivi tickets (Mantis), Structures
  (dictionnaire de données) ; Import Lots → voir ci-dessus.
...
| 2     | Écriture commerciale + droits fins                | ✅ 2026-08-13 — table `droit` (235 restrictions legacy) + `requireDroit` serveur + `useDroitsComm` client ; modales Réserver/Modifier (fiche complète) et Annuler (historique conservé), CRUD versements DG, propagations adresse/date livraison (lots investisseurs exclus, dates saisies conservées) ; `prestataire_comm1/2` repris ; adresse de tranche modifiable (FEN_Fiche_Tranche_Adresse) et export CSV d'interface de l'opération (BTN_Exporter — 62 colonnes iso-`REQ_InterfaceCommercialisation_Lot`, toutes tranches, hors famille AUTRES). TMA neutralisé dans le WinDev source ; motif d'annulation en texte libre (à trancher client)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 3     | Suivi de production (Opérations)                  | ✅ 2026-08-13 — stades courants de la tranche (actuel/prochain + suivi) et chargés d'opération repris (migration 0019) et affichés (fiche Opérations, entête SAV, onglet Stade). Alertes : tables legacy vides, non repris ; Contentieux repris le 2026-08-13 (onglet CRUD par opération + ETL, table legacy vide) avec bouton Modifier notaires/architectes ; Synchro dates + planning/export → phase 9                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
