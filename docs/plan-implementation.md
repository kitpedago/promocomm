...
| 8     | Déclarations et Bilan                             | 🔶 2026-08-13 — Bilan par SCCV livré : ETL `tBilan_Stock`/`tBilan_CAHT`/`tBilan_Resultat` (428/207/607 lignes), écran `/bilan` (volet SCCV filtrable, accordéons Stock et CA / Résultats / IS - Non IS, CRUD par modales, colonnes calculées iso-WinDev dont quotes-parts × % KPI). Reste : Déclarations (Assurance DO/MRH, SGA, 940 & LASM)                                                                       |
